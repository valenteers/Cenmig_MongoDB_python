...
def replace_record(csv_update, index_column, filename_backup, upsert = False):
    ...
    for _ , row in new_data.iterrows():
        ...
        if use_id == True:
            dbCollection.replace_one({index_column : ObjectId(row['_id'])}, {'$set' : update_dict}, upsert= upsert)
        elif use_id == False:
            dbCollection.replace_one({index_column : str(row[index_column])}, {'$set' : update_dict}, upsert= upsert)
