...
def download_ncbi_SEA(file_out = setting.dataport, db_path = setting.data_sequences):
    db_metadata = util.connect_database()
    db_metadata = db_metadata.find(setting.analyze_query)
    db_metadata = pd.DataFrame.from_dict(db_metadata)
    df = db_metadata[['Run', 'Organism', 'cenmigID', 'asm_acc', 'ftp_path']]
    ...
    file_in_check = file_out
    check_missing_sequence_file(df, file_in_check, db_path)
    
    return db_metadata # Return this for store_fileSEA func
    
def check_missing_sequence_file(df, file_in_check, db_path):
    # file_in_check == file_out
    file_out = file_in_check
    
    df_sra = df
    df_sra = df_sra[['Run','cenmigID', 'Organism']].dropna()
    df_sra['Run_check'] = df['Run'].astype(str) + '.sra'
    
    # Check available SRA
    sra_file = glob.glob(file_in_check + '*.sra')
    
    # Check if no SRA in db
    db_sra_file_list = glob2.glob(db_path + '*/*/*.sra')
    db_sra_file_list = [i.rsplit('/', 1)[1] for i in db_sra_file_list]
    df_sra = df_sra.loc[~df_sra['Run_check'].isin(db_sra_file_list)]
    df_sra = df_sra.loc[~df_sra['Run_check'].isin(sra_file)]
    ...
    df_fasta = df 
    df_fasta = df_fasta[['asm_acc','cenmigID', 'Organism', 'ftp_path']].dropna()
    df_fasta['asm_acc_check'] = df['asm_acc'].astype(str) + '.fa'
    
    # Check available SRA
    fasta_file = glob.glob(file_in_check + '*.fa')
    
    # Check if no SRA in db
    db_fasta_file_list = glob2.glob(db_path + '*/*/*.fa')
    db_fasta_file_list = [i.rsplit('/', 1)[1] for i in db_fasta_file_list]
    df_fasta = df_fasta.loc[~df_fasta['asm_acc_check'].isin(db_fasta_file_list)]
    df_fasta = df_fasta.loc[~df_fasta['asm_acc_check'].isin(fasta_file)]
...
def download_from_ncbi(csv, file_out , db_path = setting.data_sequences):
    filetype = input('Which file type to download (SRA, Fasta or all) : ').lower()
